...
- 下节课内容与重点倾听内容备忘
- 双入口排课日历：可在总日历点击日期快速排课，也可从学生档案直接查看、添加和编辑该学生课程；每名学生可设置任意多条固定周课时，支持跨多天、同一天多节及连续快速添加
- 可随时增加、删除自定义档案栏
- 数据自动保存在当前浏览器的本地存储中
...
数据中心中的同步队列会区分“等待同步”“等待重试”和“同步失败”，并显示记录的中文类型、尝试次数以及加入／最近尝试时间。同步开始时只在队列顶部显示“同步进行中”；没有明确活动标识的单条记录仍标为等待，避免误导。只有仍保存在本机、尚未上传的附件可以取消；已经上传到云端的附件记录不会显示取消按钮。错误只按登录、网络、权限、空间、重试超限或未知问题显示固定中文建议，不会把服务端原文、邮箱、令牌、网址或文件路径写入页面。
